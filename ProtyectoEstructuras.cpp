...
bool esNumericoD(string linea)
{
    bool b = true;
    double longitud = linea.size();
...
void MD(stack<SacoCafe>& T, int i)
{
    ...
    while (!Copia.empty())
    {
        ...
        cout << "|Identifcador de la entrega: " << Sa.Id_Entrega << endl;
    ...
    if (actual != NULL)
    {
        ...
        do
        {
            if (Comparar(actual->dato, ref))
            {
                ...
                cout << "\nEl nombre actual es: " << ref.Nombre << " El nuevo sera: "; cin >> T.Nombre; 
                actual->dato.Nombre = T.Nombre; cin.ignore();
                ...
                do
                {
                    cout << "\NLa canitidad de viajes realizada es: " << ref.cantidad_viajes << " nueva cantidad de viajes: ";
                    getline(cin, viaje);
                    if (esNumerico(viaje))
                    {
        ...
        } while (actual != pInicio && encontrado != true);
    }
    else
    {
